...
import requests
from osbot_utils.utils.Http                                                                     import is_port_open, url_join_safe, wait_for_port, wait_for_http, wait_for_port_closed
...
class Schema__Server__API__Send_SGraph_AI__Config(Type_Safe):
    ...
    health_check__api__path            : Safe_Str__Url__Path     = SEND_SGRAPH_AI__SERVER__API__PATH__HEALTH_CHECK
...
class Server__API__Send_SGraph_AI(Type_Safe):
    ...
    def config__set_server_status(self):
        with self.config as _:
            if _.health_check__port__last_status and _.health_check__api__last_status:
    ...
    def url__for_server(self, path=''):
        with self.config as _:
            ...
            return url_join_safe(base_path, path)

    def url__for_server__health_check(self):
        with self.config as _:
            return self.url__for_server(_.health_check__api__path)
    ...
    def server__start(self):
        self.config__update()         # todo: add context support here so that we don't need to always have to have both .config__load() and .config__save() in cases like this
        with self.config as _:
    ...
    def server__stop(self):
        ...
        with self.config as _:
            if self.server__is_running():
                stop_process(_.server__process_id)              # todo: add option to wait for port to be closed
                _.server__stopped      = True
                self.wait_for__port__closed()
    ...
    def wait_for__server(self):
        with self.config as _:
            url = self.url__for_server__health_check()
